...
import './AITools.css'; // CORRECT: This is the only CSS import needed
...
const QuizGenerator = () => {
  ...
  const handleGenerateQuiz = async (e) => {
    ...
    try {
      ...
      if (response.data.success) { setQuiz(response.data.quiz); toast.success('Quiz generated successfully!'); } else { setError(response.data.message || 'Failed to generate quiz.'); toast.error(response.data.message || 'Failed to generate quiz.'); }
  ...
  const handleSubmitQuiz = async () => {
    ...
    setShowResults(true); const score = calculateScore(); const totalQuestions = quiz.length; const userId = user?._id;
    if (!userId) { toast.error("Could not find user. Please ensure you're logged in."); return; }
    try {
      const response = await axios.post(`${server}/api/submit-quiz-result`, { userId, topic, difficulty, score, totalQuestions });
...
const FormulaGenerator = () => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-inner w-full">
      ...
      <form onSubmit={handleGenerateFormulas} className="space-y-5 mb-8 max-w-lg mx-auto">
        ...
        <button type="submit" className="w-full bg-blue-600 text-white py-3 px-4 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 flex items-center justify-center font-semibold" disabled={loading}>{loading ? <FaSpinner className="animate-spin mr-3" /> : 'Generate Formulas'}</button>
      </form>
      {error && <div className="mt-6 p-4 bg-red-100 text-red-700 rounded-lg"><p>{error}</p></div>}
      {formulas && formulas.length > 0 && (<div className="mt-8 formula-display space-y-4"><h3 className="text-xl font-bold text-gray-800 mb-4 text-center">Generated Formulas:</h3>{formulas.map((f, index) => (<div key={index} className="p-4 border border-gray-200 rounded-lg bg-gray-50 formula-card"><p className="font-bold text-md text-gray-900 mb-1">{f.name}</p><p className="font-mono text-lg text-indigo-700 bg-gray-100 p-2 rounded-md mb-2 overflow-x-auto">{f.formula}</p><p className="text-gray-700 text-sm">{f.description}</p></div>))}</div>)}
    </div>
  );
};
...
const RecommendationEngine = () => {
  ...
  useEffect(() => {
    const fetchRecommendation = async () => {
      ...
      } catch (err) { console.error("Error fetching recommendation:", err); setError(err.response?.data?.message || 'An unexpected error occurred.'); toast.error(err.response?.data?.message || 'An unexpected error occurred.');
      } finally { setLoading(false); }
    };
    fetchRecommendation();
  }, [user]);
